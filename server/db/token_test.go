...
func TestToken(t *testing.T) {
	...
	t.Run("OneTimer", func(t *testing.T) {
		...
		// Try and use again.
		if err := tx.useToken(k.Token); !errors.Is(err, smolboard.ErrUnknownToken) {
...
func TestTokenListAdmin(t *testing.T) {
	...
	var tokens = make([]smolboard.Token, 0, 15)
	...
	admin := testPermissionSet[smolboard.PermissionAdministrator].begin(t, d, owner)
...
func TestTokenCreateDeny(t *testing.T) {
	...
	for perm, test := range testPermissionSet {
		t.Run(perm.String(), func(t *testing.T) {
			// Skip owner.
			if perm == smolboard.PermissionOwner {
				return
			}
			...
			if perm == smolboard.PermissionAdministrator {
				return
			}
...
func TestTokenTrustedDelete(t *testing.T) {
	...
	user := testPermissionSet[smolboard.PermissionTrusted].begin(t, d, owner)
	tx := testBeginTx(t, d, user.AuthToken)

	if err := tx.DeleteToken(token); !errors.Is(err, smolboard.ErrActionNotPermitted) {
...
func TestTokenAdminDelete(t *testing.T) {
	...
	test := testPermissionSet[smolboard.PermissionAdministrator]
	s := test.begin(t, d, owner)
	...
	t.Run("Others", func(t *testing.T) {
		admin := newTestUser(t, d, owner.AuthToken, "ヒメゴト", smolboard.PermissionAdministrator)
		token := testOneTimeToken(t, d, admin.AuthToken)
